...
import { ChevronLeft, Star, MessageSquare, ThumbsUp, MoreVertical, Edit2, Trash2, Image as ImageIcon } from 'lucide-react';
...
const REVIEWS_DATA = [
  {
    ...
    rating: 5,
    content: '바다를 보면서 걷는 기분이 정말 최고였어요! 골목골목 숨겨진 카페들도 너무 예쁘고, 사진 찍기 너무 좋은 곳입니다. 노을 질 때쯤 가는 걸 강력 추천드려요.',
    date: '2024.03.25',
    likes: 12,
  ...
  },
  {
    ...
    courseImage: '/images/jeju-east.png',
    rating: 4,
    content: '도심 속에서 자연을 느낄 수 있는 최고의 장소입니다. 다만 주말에는 사람이 조금 많아서 평일 오전에 방문하는 것이 더 여유로울 것 같아요.',
    date: '2024.03.20',
    likes: 8,
    images: [],
  },
  {
    ...
    courseImage: '/images/jeju-east.png',
    rating: 5,
    content: '야경이 정말 아름다워요. 경복궁 주변을 돌며 역사의 숨결을 느낄 수 있었습니다. 외국인 친구와 함께 왔는데 너무 좋아하네요.',
    date: '2024.03.15',
    likes: 24,
...
const STATS = [
  { label: '전체 리뷰', value: '12', icon: MessageSquare, color: 'text-purple-600', bgColor: 'bg-purple-50' },
  { label: '평균 별점', value: '4.8', icon: Star, color: 'text-yellow-500', bgColor: 'bg-yellow-50' },
  { label: '도움됨', value: '44', icon: ThumbsUp, color: 'text-blue-500', bgColor: 'bg-blue-50' },
];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {STATS.map((stat, i) => {
            ...
            return (
              ...
              >
                <div className={`mb-4 p-3 rounded-2xl ${stat.bgColor} ${stat.color}`}>
                ...
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="text-lg font-bold text-gray-900 truncate pr-8">{review.courseName}</h3>
                      <div className="relative">
                        ...
                        {activeMenu === review.id && (
                          <div className="absolute right-0 mt-2 w-32 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-10">
                            <button className="flex items-center gap-2 w-full px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                              <Edit2 className="h-4 w-4" /> 수정하기
                            </button>
                            <button className="flex items-center gap-2 w-full px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50 transition-colors">
                ...
                {review.images.length > 0 && (
                  <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hide mb-4">
                    {review.images.map((img, i) => (
                      <div key={i} className="relative h-32 w-32 rounded-2xl overflow-hidden border border-gray-100 flex-shrink-0">
